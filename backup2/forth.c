...
static void Execute(cell address) {
  //fprintf(stderr, "exe(%x,%x,%d) ", (unsigned int) address, (unsigned int)data_stack_pointer, data_stack_pointer - data_stack);
  asm __volatile__ (
    "pushl %%ebp;"
    "movl %%eax, %%ebp;"
    "movl (%%ebp), %%ebx;"
  ...
  //fprintf(stderr, "-exe(%x,%x) ", (unsigned int) address, (unsigned int)data_stack_pointer);
}
...
static void CompileInstruction(cell val1, cell val2, cell format) {
  ...
  if (val1 != 0x90 || format != 0x1) {
    fprintf(stderr, "CompileInstr(%x,%x,%x) ", (unsigned int)val1, (unsigned int)val2, (unsigned int)format);
  }
...
static void CompileCall(cell address) {
  ...
  fprintf(stderr, "CompileCall %x (%x)\n", (unsigned int) address, (unsigned int) (address - here));
}
